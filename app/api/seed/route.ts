...
export async function GET() {
  try {
    ...
    let catMacarrao = await prisma.category.findFirst({
      where: { organizationId: org.id, name: { contains: "Clássicos da Casa Pires", mode: "insensitive" } }
    });
    if (!catMacarrao) {
      catMacarrao = await prisma.category.create({
        data: { name: "Clássicos da Casa Pires", organizationId: org.id, sequence: 1 }
      });
    }

    // Ensure category "Crocantes da Casa Pires" exists
    let catFrangos = await prisma.category.findFirst({
      where: { organizationId: org.id, name: { contains: "Crocantes da Casa Pires", mode: "insensitive" } }
    });
    if (!catFrangos) {
      catFrangos = await prisma.category.create({
        data: { name: "Crocantes da Casa Pires", organizationId: org.id, sequence: 2 }
      });
    }

    // Ensure category "Bebidas" exists
    let catBebidas = await prisma.category.findFirst({
      where: { organizationId: org.id, name: { contains: "Bebidas", mode: "insensitive" } }
    });
    if (!catBebidas) {
      catBebidas = await prisma.category.create({
        data: { name: "Bebidas", organizationId: org.id, sequence: 3 }
      });
    }
    ...
    return NextResponse.json({ success: true, message: "Seed completado com sucesso :)" });
  } catch (error: any) {
    return NextResponse.json({ success: false, message: error?.message || "Erro no seed" }, { status: 500 });
  }
}
